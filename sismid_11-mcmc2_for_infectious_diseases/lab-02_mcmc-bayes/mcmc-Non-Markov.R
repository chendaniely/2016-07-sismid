...
mcmcSIR.Gamma <- function(data, iter, alpha) {
  ...
  for (i in 2:iter) {
    ...
    if (log.prod.can != -Inf) { 
      ...
      log.pi.can <- log.prod.can - (beta.cur/N)*double.sum.can - gamma.cur*sum.R.minus.I.can + (alpha-1)*sum.log.R.minus.I.can;
      log.pi.cur <- log.prod.cur - (beta.cur/N)*double.sum.cur - gamma.cur*sum.R.minus.I.cur + (alpha-1)*sum.log.R.minus.I.cur;
